...
export default async function TransactionsPage() {
  ...
  const [{ data: txData }, { data: banksData }, { data: catsData }] = await Promise.all([
    admin
      .from("transactions")
    ...
    admin.from("banks").select("*").eq("user_id", authUser.id).order("name"),
    admin.from("categories").select("*").eq("user_id", authUser.id).order("name"),
  ]);
